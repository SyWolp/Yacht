...
const Game = () => {
  ...
  const sumDice = () => {
    ...
    setResultSum({
      ...
      bottom: {
        'Four of a Kind':
          result.filter((v, i, a) => v === a[i + 1]).length === 3 &&
          result
            .filter((v, i, a) => v === a[i + 1])
            .every((vv, ii, aa) => vv === aa[0])
            ? Array.from(
                { length: 4 },
                (_, x) => result.filter((v, i, a) => v === a[i + 1])[0],
              ).reduce((a, b) => a + b)
            : result.every((v) => v === result[0])
            ? result[0] * 4
            : 0,
        'Full House':
          result.filter((v, i, a) => v === a[i + 1]).length === 3
            ? result
                .filter((v, i, a) => v === a[i + 1])
                .every((vv, i, a) => vv === a[0]) === false
              ? result.reduce((a, b) => a + b)
              : 0
            : 0,
        'Little Straight':
          result
            .filter((v, i) => result.indexOf(v) === i)
            .sort((a, b) => b - a)
            .filter((v, i, a) => v - 1 === a[i + 1])
            .filter((vvv, iii, aaa) => vvv - 1 === aaa[iii + 1]).length === 2
            ? 15
            : result
                .filter((v, i) => result.indexOf(v) === i)
                .sort((a, b) => b - a)
                .filter((v, i, a) => v - 1 === a[i + 1])
                .filter((vvv, iii, aaa) => vvv - 1 === aaa[iii + 1]).length ===
              3
            ? 15
            : 0,
        'Big Straight':
          result.sort((a, b) => b - a).filter((v, i, a) => v - 1 === a[i + 1])
            .length === 4
            ? 30
            : 0,
  ...
  return (
    <Flex h={'9xl'} justifyContent={'space-around'}>
      ...
      <Score
        ...
        nowPlayer={nowPlayer}
        resultDice={resultDice}
        playerScore={playerScore.second}
        reusltSum={reusltSum}
      />
    </Flex>
  )
